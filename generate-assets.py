"""Render the profile hero and stack. Run with uv run generate-assets.py."""

import re
import xml.etree.ElementTree as ET

from scripts.artwork import ROOT, Canvas

ASSETS = ROOT / "assets"
ASSETS.mkdir(exist_ok=True)


def hero(theme: str, mobile: bool) -> None:
    w, h = (520, 670) if mobile else (960, 450)
    c = Canvas(
        w,
        h,
        theme,
        "Eden / AI Agent Engineering",
        "Building agents from the inside out. An animated, illustrative agent workbench: context, tools, sessions, and evaluation.",
    )
    p = c.p
    c.add("""<style>
    ...
    c.add(
        f'<defs><radialGradient id="wash"><stop stop-color="{p["blue"]}" stop-opacity=".2"/><stop offset="1" stop-color="{p["bg"]}" stop-opacity="0"/></radialGradient><pattern id="grid" width="24" height="24" patternUnits="userSpaceOnUse"><circle cx="1" cy="1" r=".8" fill="{p["line"]}"/></pattern></defs>'
    )
    c.rect(0, 0, w, h, "url(#grid)", 18)
    c.add(
        f'<ellipse cx="{w * 0.75}" cy="{h * 0.6}" rx="360" ry="250" fill="url(#wash)"/>'
    )
    ...
    c.text(
        "eden",
        28,
        203 if mobile else 228,
        138 if mobile else 158,
        display="jetbrains-bold",
    )
    ...
    c.rect(
        0, 0, 395 if not mobile else 460, 237, p["panel"], 12, f'stroke="{p["line"]}"'
    )
    ...
    c.add(
        f'<path d="{route}" stroke="{p["line"]}" stroke-width="2" fill="none"/><path d="{route}" stroke="{p["cyan"]}" stroke-width="3" fill="none" class="signal"/>'
    )
    for label, nx, ny, color in [
        ("context", 90, 85, p["blue"]),
        ("tools", 305, 85, p["cyan"]),
        ("session", 90, 184, p["pink"]),
        ("evaluate", 305, 184, p["blue"]),
    ]:
        c.rect(
            nx - 56,
            ny - 17,
    ...
    c.text(
        "Open to Agent Engineering roles", 49, h - 21, 13 if mobile else 14, p["muted"]
    )
    if not mobile:
...
def stack(theme: str, mobile: bool) -> None:
    w, cols, tilew, tileh = (520, 3, 157, 105) if mobile else (960, 5, 180, 98)
    h = 70 + ((len(ICONS) + cols - 1) // cols) * tileh + 18
    c = Canvas(
        w, h, theme, "Tools I build with", ", ".join(label for _, label in ICONS)
    )
    p = c.p
    ...
    for i, (name, label) in enumerate(ICONS):
        ...
        c.rect(
            ...
            p["panel"],
            9,
            f'stroke="{p["line"]}" stroke-width=".6"',
        )
        icon = ET.parse(ROOT / "vendor" / "devicon" / f"{name}.svg").getroot()
        icon.attrib.update(
            x=str(x + (tilew - 12 - 34) / 2), y=str(y + 13), width="34", height="34"
        )
        ...
        for ident in ids:
            s = s.replace(f'id="{ident}"', f'id="{name}-{ident}"').replace(
                f"#{ident}", f"#{name}-{ident}"
            )
        if name in ("rust", "nextjs") and theme == "dark":
            s = s.replace('fill="#000"', 'fill="#EDF0FF"').replace(
                'fill="#000000"', 'fill="#EDF0FF"'
            )
            s = (
                s.replace("<path ", '<path fill="#EDF0FF" ', 1)
                if "fill=" not in s
                else s
            )
        c.add(s)
...
def contact(theme: str, mobile: bool) -> None:
    ...
    c = Canvas(
        ...
        "Coding agents, developer tools, and the people building them.",
    )
    p = c.p
    c.rect(0, 0, w, h, p["bg"], 14)
    c.add(
        f'<path d="M24 24 H56 M24 24 V50 M{w - 24} {h - 24} H{w - 56} M{w - 24} {h - 24} V{h - 50}" stroke="{p["cyan"]}" stroke-width="2" fill="none"/>'
    )
    c.text(
        "Let's build",
        w / 2,
        63 if mobile else 56,
        32,
        display="jetbrains-bold",
        anchor="middle",
    )
