...
def gh(*args):
    return json.loads(subprocess.check_output(['gh','api',*args],text=True,timeout=90))

def fetch():
    cursor, stars, count = None, 0, 0
    while True:
        args = ['graphql','-f',f'query={QUERY}','-f',f'login={LOGIN}']
        if cursor: args += ['-f',f'cursor={cursor}']
        response = gh(*args)
        if response.get('errors'): raise RuntimeError('GitHub GraphQL returned errors')
    ...
    assert sum(day['contributionCount'] for day in days) == calendar['totalContributions']
    snapshot = {
        'login': LOGIN,
    ...
    print(f'Fetched {count} original public repositories; {calendar["totalContributions"]} contributions.')
